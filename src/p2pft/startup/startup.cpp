...
std::unique_ptr<IApplication> AppVisitor::operator()(cli::SenderArgs args)
{
    return std::make_unique<Sender>(args);
}

std::unique_ptr<IApplication> AppVisitor::operator()(cli::ReceiverArgs args)
{
    return std::make_unique<Receiver>(args);
}

std::unique_ptr<IApplication> AppVisitor::operator()(std::nullopt_t)
{
    return nullptr;
}
